...
const gatherControlsData = (ownObj: any, viaRelay: any) => {
  const controlsData = {
...
const gatherUpdateData = (objectIds: any, objects: any, viaRelay: any) => {
  const data: any = { type: 'update', update: {} };
  objectIds.current.forEach((oid: any) => {
    const o = objects.current[oid] && objects.current[oid].elref
      ? objects.current[oid]
      : undefined;
    if (o) {
      data.update[oid] = {
...
const handleSelf = (ownObj: any, delta: any) => {
...
const handleCamera = (state: any, ownRef: any) => {
...
const handleOwnInfoText = (text: any, ownObj: any, ownRef: any) => {
...
const Loop = ({
  connection, score, main, text, id, objectIds, objects,
}: any) => {
  ...
  useFrame((state, delta) => {
    if (ownObj) {
      ...
      if (Date.now() > nextSendTime && ownObj) {
        nextSendTime = Date.now() + (main && main === id ? sendIntervalMain : sendInterval);
        if (main && main === id) {
          sendDataOnUnorderedChannels(
